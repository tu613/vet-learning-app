...
def get_secret(key, section=None):
    try:
        ...
        st.error("🚨 ไม่พบไฟล์ .streamlit/secrets.toml กรุณาสร้างไฟล์และใส่ Key ให้ถูกต้อง")
        st.stop()
    except KeyError:
...
def create_gvcccm_context(gvcccm_data):
    if not gvcccm_data:
        return "ไม่พบข้อมูลมาตรฐาน GVCCCM"
...
def final_evaluation(conversation_history, gvcccm_context, score_context):
    ...
    try:
        # สร้าง Client โดยดึง Key จาก Secrets
        ...
        with st.spinner("🧠 AI กำลังวิเคราะห์ผลการซักประวัติ..."):
            ...
            prompt_content = f"ประวัติการสนทนา:\n{history_text}\n\nประเมินผลตามคำสั่ง"
...
def case_selection_page():
    ...
    # ตัวอย่างเคส (ในอนาคตควรดึงจาก MongoDB Collection: case_scenario)
    cases = [
    ...
    for case in cases:
        with st.container(border=True):
            ...
            if c2.button("เริ่มฝึก", key=case['id']):
                # เตรียม Config สำหรับ AI เจ้าของสัตว์
                sys_instruct = (
                    f"คุณคือเจ้าของสัตว์ในเคส: {case['name']} บุคลิก: {case['owner_persona']} "
                    "จงตอบคำถามนักศึกษาตามบทบาท ห้ามหลุดบท ห้ามให้คะแนน"
                )
                st.session_state.owner_config = genai.types.GenerateContentConfig(system_instruction=sys_instruct)
...
def chat_page(gvcccm_context, score_context):
    ...
    if prompt := st.chat_input("พิมพ์คำถามของคุณ..."):
        ...
        try:
            # เรียก AI เจ้าของสัตว์ (ใส่ Key ตรงนี้ด้วย)
            api_key = get_secret("GEMINI_API_KEY")
